#用英译中的短语文档cmn.txt做模型翻译 实现EncoderDecoderRNN 模型训练
...
def get_proc(enc_voc, dec_voc):
    ...
    def batch_proc(data):
        ...
        批次数据处理并返回
        """
        # seq2seq模型训练数据集 X(enc_ids,dec_ids) y(labels)
        # 其中 enc_ids是encoder的输入 dec_idx[:-1]是decoder的训练输入
        # labels == dec_idx[1:]
        # 其中 dec_idx[:-1]是不含'EOS'的索引序列 dec_idx[1:]是不含'BOS'的
...
if __name__ == '__main__':
    ...
    # 编码+解码（训练样本）
    dataset = list(zip(enc_data, dec_data)) #zip对位成元组(enc_i,dec_i)
    # Dataloader
...
# 下一步的工作是 回到EncoderDecoderModel中 串接encoder decoder 成整体seq2seq 并进行整体模型测试
# ↑ 完成之后 将会把EncoderDecoderModel中的原有的分开的 Encoder Decoder 测试注释掉